...
declare %private function app:expand-query($query as xs:string?, $scripts as xs:string?) as item()+ {
    if ($query) then (
        ...
        then
            (:if the user wants to search in Devanagri, then transliterate and discard original query:)
            if ($scripts = "sa-Deva") 
            then
                (translate(sarit:transliterate("roman2devnag", $query), "&#8204;", ""), "delete")
            else 
                (:if the user wants to search in both romanization and Devanagri, then transliterate and keep original query:)
                if ($scripts = "all")
                then
            ...
            if (not(string-to-codepoints($query)[empty(index-of((32, 2304 to 2431, 43232 to 43259, 7376 to 7412), .))])) 
            then
                (:if the user wants to search in romanization, then transliterate but delete original query:)
                if ($scripts = "sa-Latn") 
                then
                    (sarit:transliterate("devnag2roman", $query), "delete")
                else
                    (:if the user wants to search in both Devanagri and romanization, then transliterate but keep original query:)
                    if ($scripts = "all")
                    then
